...
Not every altimeter has GPS. `analyze_no_gps()` (fed by `load_blueraven_lr_csv()`, for a BlueRaven's barometer-only export) covers that case: real apogee altitude and real descent rates are still fully recoverable from the barometer alone, but boost-phase drift can't be *measured* without a real apogee GPS fix -- only *estimated*, by assuming the wind-only descent simulation is accurate and backing the boost-phase offset out of the difference between that simulation and a real hand-recorded landing GPS pin. That estimate is flagged end to end (`apogee.position_source`, a dedicated note in the viewer's info box) and never scored as if it were a measurement -- see the module's own docstring for why.
