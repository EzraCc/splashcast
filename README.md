...
- Shows rain, temperature, and per-model cloud cover (low/mid/high, waiver-aware) together in one combined weather panel below the map — a shared `Prior day | Morning | 9am | 11am | 1pm | 3pm` header whose 4 hour columns double as the map's own time-of-day selector, so there's one hour control instead of two duplicated ones. Rain sums (not just samples) the hours it doesn't show a dedicated column for into the nearest kept one, so a passing shower can't hide inside a dropped hour; temperature scales to that capture's own real range rather than a fixed scale (a Texas August day and a South Dakota April one have almost no overlap) and toggles between actual air temperature and Open-Meteo's combined "feels like" figure (default); clouds flag an hour where a majority of models agree it's at/above the safety-code cloud-cover threshold. Collapsible as a whole, plus a burn-ban status chip/overlay for sites with a real per-county feed to check.
