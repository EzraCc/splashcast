...
- Shows the resulting landing points as a **convex-hull zone** per altitude/time — not a single predicted point — shifted by a directional rail-angle offset (heading + magnitude, via a compass-dial widget, defaulting into the current ground wind direction) accounting for weathercocking during boost.
