...
- Shows a per-model rain timeline above the map — accumulated rain the day before launch and that morning (ground-wetness signals, since a soaked field/access road can cancel a launch on an otherwise dry day) plus an hourly breakdown from 8am-4pm — with chance-of-rain and amount both available on hover. A temperature timeline sits below it, same shape, scaled to that capture's own real range rather than a fixed scale (a Texas August day and a South Dakota April one have almost no overlap) — toggles between actual air temperature and Open-Meteo's combined "feels like" figure (default), which folds in wind and humidity to cover both heat-index and wind-chill conditions in one number.
...
Not every altimeter has GPS, and even one that does can lose its fix exactly when it matters most. `analyze_no_gps()` covers both: real apogee altitude and real descent rates are still fully recoverable from the barometer alone, but boost-phase drift can't be *measured* without a real apogee GPS fix -- only *estimated*, by assuming the wind-only descent simulation is accurate and backing the boost-phase offset out of the difference between that simulation and the real landing point. For a true no-GPS altimeter (`load_blueraven_lr_csv()`, BlueRaven) that landing point -- and the launch-rail position -- are hand-recorded GPS pins; for a tracker whose GPS just isn't trustworthy at apogee specifically (`load_fluctus_fbb()`, confirmed on one flight where the ascent-phase fix never got a real 3D lock) they're instead the tracker's own real fixes from moments confirmed clean. Either way the estimate is flagged end to end (`apogee.position_source`, a dedicated note in the viewer's info box) and never scored as if it were a measurement -- see the module's own docstring for why.
