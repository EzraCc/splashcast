...
- Shows the resulting landing points as a **convex-hull zone** per altitude/time — not a single predicted point — shifted by a directional rail-angle offset (heading + magnitude, via a compass-dial widget, defaulting into the current ground wind direction) accounting for weathercocking during boost. At any nonzero rail angle, a green apogee marker (same triangle a real flight's own apogee uses elsewhere in the viewer) shows exactly where apogee itself ends up before any wind-driven descent drift is added — hidden at 0° since it would just sit on top of the pad marker with nothing new to show.
