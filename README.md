...
- Shows per-model ground wind, cloud cover, rain, and temperature (in that order) together in one combined weather panel below the map, site name and waiver shown as its own heading above the map rather than buried in this (collapsible) panel — a shared `Prior day | Morning | 9am | 11am | 1pm | 3pm` header whose 4 hour columns double as the map's own time-of-day selector, so there's one hour control instead of two duplicated ones. Wind (speed/gust/direction, 10m AGL) leads the panel and grades each cell green/yellow/orange/red on a graduated scale topping out at the one real cited limit (Tripoli USC §9-3 / NAR's Model Rocket Code, 20mph sustained) — only the 4 hourly columns, no Prior day/Morning (unlike rain/temp, it doesn't carry forward the way a trend does; only the actual launch window matters). Clouds, Rain, and Temp each get their own full-width heading (Clouds' carries a "Show all altitudes" toggle, Temp's carries its actual/feels-like toggle) since a shared row label undersold how distinct each section actually is. Clouds spans more than one row (low/mid/high, waiver-aware) and flags an hour where a majority of models agree it's at/above the safety-code cloud-cover threshold. Rain sums (not just samples) the hours it doesn't show a dedicated column for into the nearest kept one, so a passing shower can't hide inside a dropped hour. Temperature scales to that capture's own real range rather than a fixed scale (a Texas August day and a South Dakota April one have almost no overlap) and toggles between actual air temperature and Open-Meteo's combined "feels like" figure (default). Collapsible as a whole, plus a burn-ban status chip/overlay for sites with a real per-county feed to check.
