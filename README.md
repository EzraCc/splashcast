...
- Shows a per-model rain timeline above the map — accumulated rain the day before launch and that morning (ground-wetness signals, since a soaked field/access road can cancel a launch on an otherwise dry day) plus an hourly breakdown from 8am-4pm — with chance-of-rain and amount both available on hover. A temperature timeline sits below it, same shape, scaled to that capture's own real range rather than a fixed scale (a Texas August day and a South Dakota April one have almost no overlap).
