...
- Lets you toggle satellite vs. road map imagery, drag the launch pad to try a nearby setup spot, and adjust the rail-angle heading/magnitude live via the compass dial — all client-side, no server.
- Apogee altitude is picked via one compact vertical slider anchored to the map's own edge (shared by 2D and 3D, not a separate sidebar control) — always directly draggable/clickable, colored per-altitude to match the ladder it summarizes, with short labels ("2k'"..."50k'") on each real option, plus unlabeled reference ticks filling the gaps (finest at 500ft, coarsening automatically on a high-waiver site or a short/mobile slider so they never crowd). A dedicated «/» chevron — not the slider itself, dragging/clicking it only ever sets the altitude — opens a fly-out panel beside it with the full ladder as a plain list, sized to its own content rather than the map's full height and scrolling internally past a real-height cap (checkbox-style: click an altitude to toggle its zone on/off, double-click to solo just that one and double-click again to bring back whatever was selected before — same rules as the Model legend below), the site's own main-deploy altitude for reference, and the zone-color picker. The readout above the slider shows the current altitude as plain text (solid background, legible over the map) until clicked, then becomes an editable "type an exact predicted apogee" field — since the drift sim runs client-side against any altitude, not just the ladder's own values, this is aimed at a flyer with one real predicted apogee rather than a launch director surveying several possible flights (who'd use the checkbox ladder instead).
- Lets you edit the active drogue/main descent rate directly (fps) — real observed rates on a given rocket/recovery setup often diverge from the defaults, and the map updates live as you type. Fast/Slow buttons alongside it fill in that preset's exact numbers in one click. Main is hard-capped at 35 fps (Tripoli USC §11-1's max landing speed), with a warning if you try to exceed it.
- Lets you toggle individual weather models on/off (click a model in the legend, like a checkbox — all start selected) or solo just one (double-click) — the zone hull is recomputed client-side from whichever subset is checked, and the weather panel's own per-model bars (wind/clouds/rain/temp) follow the same selection. The legend itself sits as a horizontal row of content-sized chips directly below the map (one row on desktop, wraps to at most two on mobile), not a sidebar column.
- A «Hide controls» toggle sits in its own row above the Launch Site/Date/View controls, so a mobile user can collapse them out of the way to see more of the map without a separate scroll.
